...
export default async function Home() {
  ...
  const africa = data.filter(country => country.region ==="counties")
...
const countries = data.map(country =>(
  <article className='border-4 rounded-lg py-6 flex flex-col items-center justify-center border-sky-800 hover:shadow-2xl'>
    ...
    <Link href={country.maps.googleMaps} target='_balnk' title={`${country.name.common} on google map`}>
    <p>
    <BiMap className='text-sky-800 text-5xl'/></p>
    </Link>
  </article>
))
  return (
    ...
    <div>
      <h1 className='flex flex-col items-center h-[50px] justify-center m-4 bg-sky-800 text-white'> Countries of the world
      </h1>
    ...
    <footer className='flex flex-col items-center h-[50px] justify-center m-4 bg-sky-800 text-white'>
      <p>{`we have ${countries.length} countries of the world`}</p>
       </footer>
    </div>
  )
}
